    ...
    <el-card class="box-card">
                ...
                >
                  ...
                  <el-card :body-style="{ padding: '10px' }">
                    <el-descriptions class="margin-top" :column="1" size="small" border>
                      <el-descriptions-item v-if="item.workUserName" label-class-name="my-label">
                        <template slot="label"><i class="el-icon-user"></i>办理人</template>
                        {{ item.workUserName }}
                      </el-descriptions-item>
                      <el-descriptions-item v-if="item.assigneeName" label-class-name="my-label">
                        <template slot="label"><i class="el-icon-user"></i>接收人</template>
                        {{ item.assigneeName }}
                      </el-descriptions-item>
      ...
      <el-dialog :title="completeTitle" :visible.sync="completeOpen" width="60%" append-to-body>
        <el-form ref="taskForm" :model="taskForm">
          <el-divider content-position="left">1、请选择下一步</el-divider>
          <el-radio-group v-model="nextFlowId" @change="nextFlowChange()" class="ml-4" style="padding-left: 45px;">
            <el-radio :label="item.nextFlowID" v-for="item in nextFlowList">
              {{ item.sequenceFlowName + "(" + item.nextFlowName + ")" }}
            </el-radio>
          </el-radio-group>
          <el-divider content-position="left" v-if="!isEnd">2、选择处理人</el-divider>
          <el-form-item prop="targetKey" v-if="!isEnd" style="padding-left: 45px;">
            ...
            <el-checkbox-group v-model="checkedUsers" @change="handleCheckedUsersChange">
              <el-checkbox v-for="item in nextFlowUserList" :label="item.userId" :key="item.userId">{{ item.userId }}
              </el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <el-divider content-position="left">3、填写{{ isReturn ? '退回意见' : '处理意见' }}</el-divider>
          <el-form-item label="审核意见" label-width="80px" prop="comment"
                        :rules="[{ required: true, message: '请输入审核意见', trigger: 'blur' }]"
                        style="padding-left: 45px;">
            <el-input type="textarea" v-model="taskForm.comment" placeholder="请输入审核意见"/>
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
          <el-button @click="completeOpen = false">取 消</el-button>
          <el-button type="primary" @click="taskComplete" v-if="!isReturn">确 定</el-button>
            <el-button type="primary" @click="taskReturn" v-if="isReturn">确 定</el-button>
        </span>
      </el-dialog>
      <!--退回流程-->
...
export default {
  ...
  methods: {
    ...
    /** 用户审批任务 */
    taskComplete() {
      if (this.checkedUsers.length === 0 && !this.isEnd) {
      ...
      if (!this.nextFlowId) {
        this.$modal.msgError("请选择下一环节!");
        return;
      }

      const nextFlow = this.nextFlowList.filter(e => e.nextFlowID == this.nextFlowId)[0];
      if (nextFlow.type == "multiInstance") {
        this.$set(this.taskForm.variables, nextFlow.multiinstanceVars, this.checkedUsers);
      }

      this.$set(this.taskForm.variables, nextFlow.vars, this.checkedUsers.join(','));
      this.taskForm.nextUserIds = this.checkedUsers;
    ...
    /** 提交退回任务 */
    taskReturn() {
      this.taskForm.targetKey = this.nextFlowList.filter(e => e.nextFlowID == this.nextFlowId)[0].nextFlowID;
      if (this.taskForm.variables == null)
        this.taskForm.variables = {};
      //this.$set(this.taskForm.variables, "todoUserId", this.checkedUsers.join(','));
      this.taskForm.nextUserIds = this.checkedUsers;
      this.$refs["taskForm"].validate(valid => {
        if (valid) {
    ...
    nextFlowChange() {
      this.nextFlowUserList = this.nextFlowList.filter(e => e.nextFlowID == this.nextFlowId)[0].nextUserList;
      this.isEnd = this.nextFlowList.filter(e => e.nextFlowID == this.nextFlowId)[0].end;
      this.isReturn = this.nextFlowList.filter(e => e.nextFlowID == this.nextFlowId)[0].return;
      this.checkedUsers = [];
    },
    handleCheckAllChange(val) {
      this.checkedUsers = val ? this.nextFlowUserList.map(it => it.userId) : [];
      this.isIndeterminate = false;
    },
    handleCheckedUsersChange(value) {
      let checkedCount = value.length;
      this.checkAll = checkedCount === this.nextFlowUserList.length;
      this.isIndeterminate = checkedCount > 0 && checkedCount < this.nextFlowUserList.length;
    }
  },
};
</script>
