...
public class FlowInstanceController {
    ...
    @PostMapping("/startBy/{procDefId}/{userId}/{nextUserIds}")
    public AjaxResult startById(@ApiParam(value = "流程定义Id") @PathVariable(value = "procDefId") String procDefId,
                                @ApiParam(value = "流程发起人Id") @PathVariable(value = "userId") String userId,
                                @ApiParam(value = "下一步处理人Id，多个处理人使用逗号（,）分隔") @PathVariable(value = "nextUserIds")  String nextUserIds,
                                @ApiParam(value = "流程变量集合，json对象") @RequestBody Map<String, Object> variables) {
        return flowInstanceService.startProcessInstanceById(procDefId, userId, Arrays.asList(nextUserIds.split(",")), variables);
    }
