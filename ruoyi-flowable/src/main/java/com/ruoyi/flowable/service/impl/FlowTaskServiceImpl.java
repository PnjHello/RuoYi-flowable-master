...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    public AjaxResult complete(FlowTaskVo taskVo) {
        ...
        if (DelegationState.PENDING.equals(task.getDelegationState())) {
            ...
            taskService.addComment(taskVo.getTaskId(), taskVo.getInstanceId(), FlowComment.REBACK.getType(), taskVo.getComment());

            //更新任务审核人
            taskService.setVariableLocal(taskVo.getTaskId(), this.workUserParamName, taskVo.getAssignee());//更新任务审核人
            ...
            if (nextTaskList.size() == 1)
                taskService.setAssignee(nextTaskList.get(0).getId(), String.join(",", taskVo.getNextUserIds()));
            else {
                for (int i = 0; i < nextTaskList.size(); i++) {
                    taskService.setAssignee(nextTaskList.get(i).getId(), taskVo.getNextUserIds().get(i));
                }
            }
        }
    ...
    public void taskReturn(FlowTaskVo flowTaskVo) {
        ...
        try {
            taskService.setVariable(flowTaskVo.getTaskId(), "skip", false);
            for (String key : flowTaskVo.getVariables().keySet()) {
                taskService.setVariable(flowTaskVo.getTaskId(), key, flowTaskVo.getVariables().get(key));
            }
            // 1 对 1 或 多 对 1 情况，currentIds 当前要跳转的节点列表(1或多)，targetKey 跳转到的节点(1)

            //更新当前步骤审核人
            taskService.setVariableLocal(flowTaskVo.getTaskId(), this.workUserParamName, flowTaskVo.getAssignee());//更新任务审核人
            ...
            if (nextTaskList.size() > 0)
                taskService.setAssignee(nextTaskList.get(0).getId(), String.join(",", flowTaskVo.getNextUserIds()));
            ...
            if (CollectionUtils.isNotEmpty(taskList)) {
                ...
                for (Task task : taskList) {
                    if (StringUtils.isNotBlank(task.getOwner())) {
                        // 当前任务节点办理人信息
                        List<String> ownerList = List.of(task.getOwner().split(","));
                        for (String assignee : ownerList) {
                            SysUser sysUser = sysUserService.selectUserById(Long.parseLong(assignee));
                            if (sysUser != null)
                                ownerNameList.add(sysUser.getNickName());
                        }
                    }
                }
                if (ownerNameList.size() > 0) {
                    flowTask.setAssigneeName(String.join(",", ownerNameList));
                }
            } else {
                /*List<HistoricTaskInstance> historicTaskInstance = historyService.createHistoricTaskInstanceQuery().processInstanceId(hisIns.getId()).orderByHistoricTaskInstanceEndTime().desc().list();
                flowTask.setTaskId(historicTaskInstance.get(0).getId());
                flowTask.setTaskName(historicTaskInstance.get(0).getName());
                if (StringUtils.isNotBlank(historicTaskInstance.get(0).getOwner())) {
                    // 当前任务节点办理人信息
                    *//*SysUser sysUser = sysUserService.selectUserById(Long.parseLong(historicTaskInstance.get(0).getAssignee()));
                    ...
                    // 当前任务节点办理人信息
                    List<String> ownerList = List.of(historicTaskInstance.get(0).getOwner().split(","));
                    List<String> ownerNameList = new ArrayList<>();
                    for (String assignee : ownerList) {
                        SysUser sysUser = sysUserService.selectUserById(Long.parseLong(assignee));
                        if (sysUser != null)
                ...
                List<HistoricActivityInstance> hisActinsList = historyService.createHistoricActivityInstanceQuery().processInstanceId(hisIns.getId()).orderByHistoricActivityInstanceId().desc().list();
                flowTask.setTaskName(hisActinsList.get(0).getActivityName());
            }
...
//      Map<String, VariableInstance> variables = runtimeService.getVariableInstances(currentTask.getExecutionId());
    ...
    public AjaxResult todoList(FlowQueryVo queryVo) {
        ...
        TaskQuery taskQuery = taskService.createTaskQuery()
                ...
                .taskAssigneeLike("%" + sysUser.getUserId().toString() + "%")
        ...
        if (StringUtils.isNotBlank(procInsId)) {
            ...
            for (HistoricActivityInstance histIns : list) {
                if (StringUtils.isNotBlank(histIns.getTaskId())) {
                    ...
                    if (task != null && StringUtils.isNotBlank(task.getOwner())) {
                        List<String> ownerList = List.of(task.getOwner().split(","));
                        List<String> ownerNameList = new ArrayList<>();
                        for (String owner : ownerList) {
                            SysUser sysUser = sysUserService.selectUserById(Long.parseLong(owner));
                            if (sysUser != null)
                                ownerNameList.add(sysUser.getNickName());
                        }
                        if (ownerList.size() > 0) {
                            flowTask.setOwnerNames(String.join(",", ownerNameList));
                        }
                    }

                    // 当前任务节点待办人信息
                    if (StringUtils.isNotBlank(histIns.getAssignee())) {
                        List<String> assigneeList = List.of(histIns.getAssignee().split(","));
                        List<String> assigneeNameList = new ArrayList<>();
                        for (String assignee : assigneeList) {
                            SysUser sysUser = sysUserService.selectUserById(Long.parseLong(assignee));
                            if (sysUser != null)
                                assigneeNameList.add(sysUser.getNickName());
                        }
                        if (assigneeNameList.size() > 0) {
                            flowTask.setAssigneeName(String.join(",", assigneeNameList));
                        }
                    }
                    ...
                    if (workUser!=null && workUser.getValue()!=null && StringUtils.isNotBlank(workUser.getValue().toString())) {
                        List<String> workUserList = List.of(workUser.getValue().toString().split(","));
                        List<String> workUserNameList = new ArrayList<>();
                        for (String assignee : workUserList) {
                            SysUser sysUser = sysUserService.selectUserById(Long.parseLong(assignee));
                            if (sysUser != null)
                                workUserNameList.add(sysUser.getNickName());
                        }
                        if (workUserNameList.size() > 0) {
                            flowTask.setWorkUserName(String.join(",", workUserNameList));
                        }
                    }
...
//        SysUser startUser = sysUserService.selectUserById(Long.parseLong(historicProcessInstance.getStartUserId()));
     ...
     */
    @Override
    public AjaxResult getNextFlowNode(String processInstanceId, String taskId, String userId) {
        ...
        if (StringUtils.isNotBlank(processInstanceId) && StringUtils.isNotBlank(userId) && !StringUtils.isNotBlank(taskId))
            task = taskService.createTaskQuery().active().includeProcessVariables().processInstanceId(processInstanceId).taskOwnerLike("%" + userId + "%").list().stream().findFirst().orElse(null);
        ...
        if (Objects.nonNull(task)) {
            ...
            if (CollectionUtils.isNotEmpty(nextUserTask)) {
                for (UserTask userTask : nextUserTask) {
                    ...
                    flowNextDto.setNextFlowID(userTask.getId());
                    flowNextDto.setNextFlowName(userTask.getName());
                    flowNextDto.setSequenceFlowName(userTask.getAttributeValue(null, "sequenceFlowName"));
                    flowNextDto.setEnd("1".equals(userTask.getAttributeValue(null, "isEnd")));
                    flowNextDto.setReturn("return".equals(userTask.getAttributeValue(null, "sequenceFlowDesc")));
                    ...
                    if (Objects.nonNull(multiInstance)) {
                        ...
                        flowNextDto.setMultiinstanceVars(multiInstance.getInputDataItem());
                    } else {
                    ...
                    if (userTask.getCandidateUsers().size() > 0) {
                        userTypeList = userTask.getCandidateUsers().stream().map(e -> e.substring(e.lastIndexOf("{") + 1, e.lastIndexOf("}"))).collect(Collectors.toList());
                    }
                    if (userTask.getCandidateGroups().size() > 0) {
                        userTypeList = userTask.getCandidateGroups().stream().map(e -> e.substring(e.lastIndexOf("{") + 1, e.lastIndexOf("}"))).collect(Collectors.toList());
                    }

                    flowNextDto.setVars(String.join(",", userTypeList));
                    userTypeList.forEach(type -> {
                        switch (type) {
                            case "initiator":
                                flowNextDto.setNextUserList(List.of(new SysUser[]{sysUserService.selectUserById(Long.parseLong("1"))}));
                                break;
                            case "projectManager":
                                flowNextDto.setNextUserList(List.of(new SysUser[]{sysUserService.selectUserById(Long.parseLong("1"))}));
                                break;
                            case "departmentManager":
                                flowNextDto.setNextUserList(List.of(new SysUser[]{sysUserService.selectUserById(Long.parseLong("7")), sysUserService.selectUserById(Long.parseLong("8"))}));
                                break;
                            case "hr":
                                flowNextDto.setNextUserList(List.of(new SysUser[]{sysUserService.selectUserById(Long.parseLong("1"))}));
                                break;
                            case "leader":
                                flowNextDto.setNextUserList(List.of(new SysUser[]{sysUserService.selectUserById(Long.parseLong("9")), sysUserService.selectUserById(Long.parseLong("10"))}));
                                break;
        ...
        return AjaxResult.success(flowNextDtoList);
    }
