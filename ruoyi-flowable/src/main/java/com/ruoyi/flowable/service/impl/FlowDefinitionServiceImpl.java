...
//            }
    ...
    public void delete(String deployId) {
       List<ProcessDefinition> list =  repositoryService.createProcessDefinitionQuery().active().deploymentId(deployId).list();
        list.forEach(p->{
            if(historyService.createHistoricProcessInstanceQuery().processDefinitionId(p.getId()).count()>0 || runtimeService.createProcessInstanceQuery().processDefinitionId(p.getId()).count()>0)
                try {
                    throw new Exception("流程已启动实例，不能删除!");
                } catch (Exception e) {
