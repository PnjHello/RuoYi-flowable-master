...
public class FlowInstanceServiceImpl extends FlowServiceFactory implements IFlowInstanceService {
     ...
     */
    @Override
    public AjaxResult startProcessInstanceById(String procDefId, String userId, List<String> nextUserIds, Map<String, Object> variables) {
        try {
            // 设置流程发起人Id到流程中
            identityService.setAuthenticatedUserId(userId);
            //variables.put("initiator",userId);//流程发起人
            //variables.put("skip",true);//是否跳过第一步
            ...
            if (nextUserIds != null && nextUserIds.size() > 0) {
                ...
                if (nextTaskList.size() == 1)
                    taskService.setAssignee(nextTaskList.get(0).getId(), String.join(",", nextUserIds));
                else {
                    for (int i = 0; i < nextTaskList.size(); i++) {
                        taskService.setAssignee(nextTaskList.get(i).getId(), nextUserIds.get(i));
                    }
                }
            }
            ...
            e.printStackTrace();
            return AjaxResult.error("流程启动错误!" + e.getMessage());
        }
    }
}