        ...
        if (outGoingFlows != null && outGoingFlows.size() > 0) {
            ...
            for (SequenceFlow sequenceFlow : outGoingFlows) {
                ...
                if (expression == null ||
                    ...
                    ExtensionAttribute attr_Name=new ExtensionAttribute();
                    attr_Name.setName("sequenceFlowName");
                    attr_Name.setValue(sequenceFlow.getName());

                    ExtensionAttribute attr_Desc=new ExtensionAttribute();
                    attr_Desc.setName("sequenceFlowDesc");
                    attr_Desc.setValue(sequenceFlow.getDocumentation());

                    nextFlowElement.getAttributes().put("sequenceFlowName",List.of(new ExtensionAttribute[]{attr_Name}));
                    nextFlowElement.getAttributes().put("sequenceFlowDesc",List.of(new ExtensionAttribute[]{attr_Desc}));
                    ...
                    //结束节点
                        ...
                        //next(flowElements, nextFlowElement, map, nextUser);
                        UserTask userTask = new UserTask();
                        userTask.setId(nextFlowElement.getId());
                        userTask.setName(nextFlowElement.getName());
                        userTask.setAttributes(nextFlowElement.getAttributes());

                        ExtensionAttribute endAttr=new ExtensionAttribute();
                        endAttr.setName("isEnd");
                        endAttr.setValue("1");
                        userTask.getAttributes().put("isEnd",List.of(new ExtensionAttribute[]{endAttr}));
                        nextUser.add(userTask);
                    }
                }
            }
